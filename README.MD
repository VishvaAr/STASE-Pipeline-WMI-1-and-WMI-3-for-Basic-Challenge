...
Payload Injection: The attacker writes malicious data into this slot. The stale pointer now interprets this data as valid internal pointers (e.g., avatar->username).
Trigger: The program calls free() on the stale pointer again. Instead of freeing valid heap memory, it tries to free the address specified by the attacker (e.g., a stack address or kernel pointer).
